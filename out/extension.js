...
function activate(context) {
    ...
    try {
        ...
        vscode.window.showInformationMessage('SVG Previewer extension is now active!');
        console.log('Notification sent');
        // Register hover provider for SVG content
        const hoverProvider = vscode.languages.registerHoverProvider(['html', 'xml', 'svg'], new SVGHoverProvider());
        // Register command to show SVG preview
        const showPreviewCommand = vscode.commands.registerCommand('svg-previewer.showPreview', showSVGPreview);
        // Register test command
        const testCommand = vscode.commands.registerCommand('svg-previewer.hello', () => {
...
class SVGHoverProvider {
    provideHover(document, position, token) {
        ...
        console.log('SVG content found:', svgMatch.svgContent.substring(0, 50) + '...');
        ...
        hoverContent.appendMarkdown(`<div style="background: white; padding: 15px; border: 1px solid #ccc; border-radius: 6px; margin: 10px 0; text-align: center; box-shadow: 0 2px 8px rgba(0,0,0,0.1);">
    ...
    findSVGContent(document, position) {
        ...
        const svgElementRegex = /<(circle|rect|path|line|polyline|polygon|ellipse|text|g|defs|use)[^>]*\/?>/gi;
...
async function showSVGPreview() {
    ...
    const hover = await hoverProvider.provideHover(document, position, new vscode.CancellationTokenSource().token);
    if (hover) {
        // Get saved background color preference
        const savedColor = extensionContext.globalState.get('svgPreviewer.backgroundColor', '#ffffff');
        // Show the hover content in a new webview
        const panel = vscode.window.createWebviewPanel('svgPreview', 'SVG Preview', vscode.ViewColumn.Beside, {
            enableScripts: true,
        ...
        panel.webview.onDidReceiveMessage((message) => {
            switch (message.command) {
                case 'saveBackgroundColor':
                    extensionContext.globalState.update('svgPreviewer.backgroundColor', message.color);
                    break;
            }
        ...
        vscode.window.showInformationMessage('No SVG content found at cursor position');
    }
}
function getWebviewContent(svgContent, savedColor = '#ffffff') {
    return `<!DOCTYPE html>
<html lang="en">
<head>
    <meta charset="UTF-8">
